...
function populateMangaDetails(index) {
  ...
  mangaData[index].volumesInfo.forEach((volumeInfo) => {
    ...
    volumeDiv.textContent = volumeInfo.volume;
    mangaVolume.appendChild(volumeDiv);

    const ownedDiv = document.createElement("div");
    ownedDiv.classList.add("owned");
    ownedDiv.textContent = volumeInfo.owned ? "Yes" : "No";
    mangaVolume.appendChild(ownedDiv);

    const readedDiv = document.createElement("div");
    readedDiv.classList.add("readed");
    readedDiv.textContent = volumeInfo.readed ? "Yes" : "No";
    mangaVolume.appendChild(readedDiv);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput.addEventListener("input", () => {
    ...
    if (searchInputValue === "") {
      ...
      const index = mangaData.findIndex(
        (manga) => manga.series.toLowerCase() === searchInputValue.toLowerCase()
      );
      if (index !== -1) {
